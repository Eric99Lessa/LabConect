...
int main() {
    ...
    char c = ' ';
    unsigned int n_grupo = 1;
    char txData[TRANSFER_SIZE]={n_grupo, 21, 32};
            ...
            while (received == false){
                if ( my_nrf24l01p.readable() ) {
                    ...
                    if (rxData[0] == n_grupo){
                        ...
                        pc.printf("Confirmacao recebida! Payload: %d, %d, %d\r\n", rxData[0],rxData[1],rxData[2]);
                        received = true;
                    }
                    else {
                        pc.printf("Mensagem ignorada, endereco nao corresponde ao %d\n", n_grupo);
                    }
                }
            }
        }
